...
if(isset($_POST['update_profile'])){
   ...
   mysqli_query($conn, "UPDATE `users` SET user_name = '$update_name', user_email = '$update_email' WHERE user_id  = '$user_id'") or die('query failed');
   ...
   if(!empty($update_pass) || !empty($new_pass) || !empty($confirm_pass)){
      if($update_pass != $old_pass){
         ...
         mysqli_query($conn, "UPDATE `users` SET user_password = '$confirm_pass' WHERE user_id = '$user_id'") or die('query failed');
   ...
   if(!empty($update_image)){
      if($update_image_size > 2000000){
         ...
         $image_update_query = mysqli_query($conn, "UPDATE `users` SET user_image = '$update_image' WHERE user_id = '$user_id'") or die('query failed');
